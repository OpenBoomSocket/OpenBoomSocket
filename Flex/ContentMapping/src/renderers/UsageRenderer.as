package renderers
{
	...
	public class UsageRenderer extends ComboBox
	{
		...
		override public function set data(value:Object):void {
			super.data = value;
			//Alert.show(ObjectUtil.toString(value[DataGridListData(listData).dataField].serverInfo.initialData));
			if(value!=null){
				...
				//Alert.show(arrayDP.getItemAt(0).pagename)
				if(arrayDP.getItemAt(0).pagename != "~none~"){
		...
		private function viewPage(eventObj:Event):void{
			if(this.selectedItem.pageid > 0){
				var thisRequest:URLRequest = new URLRequest(Application.application.serverURL+"/"+"admintools/main.cfm?i3currenttool=124&editpageid="+this.selectedItem.pageid);
