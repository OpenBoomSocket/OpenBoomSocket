<!---
 ...
 Modified to allow GET/POST requests by Steve Smith <steve@orderedlist.com>
...
<!--- SET REQUEST TYPE (GET or POST)--->
<cfset request_type = "GET">
...
<cfscript>
		t = ucase(request_type);
		if (t neq "GET" and t neq "POST") 
...
<cfoutput>
		...
		function sajax_do_call(func_name, url, args) {
			...
			if (sajax_request_type == "GET") {
				...
				post_data = "rsargs=";
				for (i = 0; i < args.length-1; i++) 
					post_data = post_data + escape(args[i]) + escape("#JSStringFormat(sajax_url_array_delim)#");
			}
			x = sajax_init_object();
			x.open(sajax_request_type, url, true);
			...
			x.send(post_data);
			sajax_debug(func_name + " url = " + url + "/post = " + post_data);
...
<cfscript>
	...
	if (not isdefined("url.rs")) return;
	if(ucase(request_type) eq "GET") {
		...
		if(not isdefined("url.rsargs"))
			...
			func_name = url['rs'];
			if(not isdefined("form.rsargs"))
				result = call_user_func(form['rs']);
			else
				result = call_user_func_array(url['rs'], get_url_array(form['rsargs']));
	}
