<!--- FRONTEND: i_precommit.cfm --->
<!--- Note: a different version exists in i3Global admin includes so be aware if any modifications need to be made in both places --->

<!--- all possible guest table name fields should be guest firstname & lastname --->
<cfset allGuestNameFields = "guestname,guestaccessname,guestaddressname,guestemailaddressname,guestphonename,#form.tablename#name">
...
<cfset guestObj = CreateObject('component','#application.CFCpath#.guest')>
<cfset q_guestInfo = guestObj.getGuest(guestid=thisGuestID)>
<cfparam name="FORM.rememberme" default="0">
<cfcookie name = "guestuuid" value = "#q_guestInfo.guestuuid#">
<cfcookie name = "rememberme" value = "#Trim(FORM.rememberme)#">