...
<!--- DATABASE FUNCTIONS --->
	...
	<cffunction access="remote" name="getFilesForCategory" output="false" returntype="query" displayname="get Files For Category">
		...
		<cfargument name="active" type="boolean" required="no" default="1" displayname="active">
		<cfset q_columns = getColumnList()>
		<cfif listFindNoCase(q_columns.columnlist,"ordinal")>
			<cfset useordinal = true>
		</cfif>
		<cftry>
			<cfquery datasource="#application.datasource#" name="q_getfiles" username="#APPLICATION.dbUserName#" password="#APPLICATION.dbpassword#">
				SELECT
					...
					<cfif isDefined('useordinal')>, u.ordinal</cfif>
				FROM
					upload u LEFT JOIN
						uploadcategory uc ON u.uploadcategoryid = uc.uploadcategoryid
				WHERE (0=0) AND (isNull(u.active,1) = #ARGUMENTS.active#)
				...
				ORDER BY <cfif isDefined('useordinal') AND (listFirst(arguments.sortby) EQ "ordinal")>u.ordinal  #listLast(arguments.sortby)# <cfelseif listFirst(arguments.sortby) NEQ "ordinal">#listFirst(arguments.sortby)# #listLast(arguments.sortby)#</cfif>
			</cfquery>
				<cfcatch type="database">
					<cfrethrow>
				</cfcatch>
	...
	<cffunction access="public" name="deleteFileQuery" output="false" returntype="boolean" displayname="delete File">
		...
		<cfif thisFileData.uploadcategoryid EQ '99999'>
			<cftry>
				...
				<cfquery name="q_deletefile" datasource="#application.datasource#">
					DELETE FROM upload
...
<!--- FILESYSTEM FUNCTIONS --->
	...
	<cffunction access="public" name="createThumbNail" output="false" returntype="void" displayname="Create a thumbnail">
		...
		<cfargument name="resizeOnValue" type="boolean" required="yes" displayname="resizeOnValue" hint="1 = width, 0 = height" default="1">
		<cfargument name="imageThumbSize" type="numeric" required="yes" displayname="imageThumbSize" hint="Heigth of thumbnail">
		<cfargument name="aspectRatio" type="boolean" required="no" displayname="aspect Ratio" hint="Keeps image proportional" default="1">
		<cfset var thisWidth=''>
		<cfset var thisHeight=''>
	
		<!--- This instantiates the componenet, thereby creating an object for us to work with. --->
		<cfset var myImage = CreateObject("Component", "#APPLICATION.cfcpath#.util.imageCFC.image") />
		...
		<cfif resizeOnValue>
			<cfset thisWidth = imageThumbSize>
			<cfset thisHeight = 0>
		<cfelse>
			<cfset thisWidth = 0>
			<cfset thisHeight = imageThumbSize>		
		</cfif>

		<!--- resize the image to a specific width and height --->
		<cfset thisImage = myImage.resize(objImage='',inputFile='#arguments.dirpath#\#arguments.filename#',outputFile='#arguments.dirpath#\#thisfileName#_thumb.#thisfileExtention#',newWidth=thisWidth,newHeight=thisHeight,preserveAspect=arguments.aspectRatio) />
	</cffunction>

	<cffunction access="public" name="getColumnList" output="false" returntype="query">
		<cftry>
			<cfquery name="q_getColumnList" datasource="#APPLICATION.datasource#" username="#APPLICATION.dbUserName#" password="#APPLICATION.dbpassword#">
				SELECT *
				FROM upload
				WHERE (0=1)
			</cfquery>
