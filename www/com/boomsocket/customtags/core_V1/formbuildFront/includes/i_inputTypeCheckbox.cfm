	...
	<tr>
		<cfloop from="1" to="#colNum#" index="thisCol">
			...
			<td valign="top" class="#evaluate("a_tableelements[#a1#].cell_#a2#.class")#"<cfif nowrap> nowrap="nowrap"</cfif>>
				...
				<cfif thisHereRecord LTE maxRecords><input type="checkbox" name="#a_formelements[a].fieldname#" id="#a_formelements[a].fieldname#" value="#q_getlist.lookupkey[thisHereRecord]#~#q_getlist.lookupdisplay[thisHereRecord]#"<cfif listContainsNoCase(evaluate('form.#a_formelements[a].fieldname#'),"#q_getlist.lookupkey[thisHereRecord]#")> checked</cfif> #javascriptFunction#<cfif a_formelements[a].readonly EQ 1> readonly="readonly"</cfif><cfif len(a_formelements[a].tabindex)> tabindex="#a_formelements[a].tabindex#"</cfif>> #q_getlist.lookupdisplay[thisHereRecord]#<cfelse>&nbsp;</cfif>
			</td>
		</cfloop>
	</tr>
	</cfloop>
...
<cfelseif a_formelements[a].lookuptype eq "table"><!--- must return 2 query vars: lookupdisplay, lookupkey --->
	...
	<tr>
		...
		<cfset a2=listLast(a_formelements[a].gridposvalue,'_')>
			<td valign="top" class="#evaluate("a_tableelements[#a1#].cell_#a2#.class")#"<cfif nowrap> nowrap="nowrap"</cfif>>
				<cfset thisHereRecord=val(thisRow+((thisCol-1)*rowNum))>
				<cfif thisHereRecord LTE maxRecords><cfset fieldValue="#q_getlist.lookupkey[thisHereRecord]#~#q_getlist.lookupdisplay[thisHereRecord]#">
				<cfset thisFormFieldValueList="#evaluate('form.#a_formelements[a].fieldname#')#"></cfif>
				<cfif thisHereRecord LTE maxRecords><input type="checkbox" name="#a_formelements[a].fieldname#" id="#a_formelements[a].fieldname#" value="#fieldValue#"<cfif listContainsNoCase(thisFormFieldValueList,"#q_getlist.lookupkey[thisHereRecord]#")> checked</cfif> #javascriptFunction#<cfif a_formelements[a].readonly EQ 1> readonly="readonly"</cfif><cfif len(a_formelements[a].tabindex)> tabindex="#a_formelements[a].tabindex#"</cfif>> #q_getlist.lookupdisplay[thisHereRecord]#<cfelse>&nbsp;</cfif>
			</td>
		</cfloop>
	</tr>
	</cfloop>
...
<cfelse><!--- must be a 2 delimiter list (key,value;) --->
	...
	<tr>
		...
		<cfset a2=listLast(a_formelements[a].gridposvalue,'_')>
			<td valign="top" class="#evaluate("a_tableelements[#a1#].cell_#a2#.class")#"<cfif nowrap> nowrap="nowrap"</cfif>>
				<cfset thisHereRecord=val(thisRow+((thisCol-1)*rowNum))>
				<cfif thisHereRecord LTE maxRecords><input type="checkbox" name="#a_formelements[a].fieldname#" id="#a_formelements[a].fieldname#" value="#listFirst(listGetAt(a_formelements[a].lookuplist,thisHereRecord,';'),',')#~#listLast(listGetAt(a_formelements[a].lookuplist,thisHereRecord,';'),',')#"<cfif listContainsNoCase(evaluate('form.#a_formelements[a].fieldname#'),"#listFirst(listGetAt(a_formelements[a].lookuplist,thisHereRecord,';'),',')#")> checked</cfif> #javascriptFunction#<cfif a_formelements[a].readonly EQ 1> readonly="readonly"</cfif><cfif len(a_formelements[a].tabindex)> tabindex="#a_formelements[a].tabindex#"</cfif>> #listLast(listGetAt(a_formelements[a].lookuplist,thisHereRecord,';'),',')#<cfelse>&nbsp;</cfif>
			</td>
		</cfloop>
	</tr>
	</cfloop>
