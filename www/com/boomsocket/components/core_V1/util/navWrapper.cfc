...
AUTHOR:         Darin Kohles support@d-p.com
...
<cfif NOT arguments.subsOnly>
	...
	<cffunction access="public" name="setSelectedXML" output="true" returntype="string" displayname="setSelected" hint="sets class for selection status">
		...
		<cfloop from="1" to="#arrayLen(navXML)#" index="r">
            ...
            <cfelse>
                <cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug A<cfdump var="#thisNode#" label="thisNode"></cfmail>
                <cfset linkXML.XMLAttributes = thisNode.XMLChildren[1]['a'].XMLAttributes>
            </cfif>
			...
			<cfelse>
                ...
                <cfelse>
                    <cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug B<cfdump var="#thisNode#" label="thisNode"></cfmail>
                    <cfset linkXML.XMLText = thisNode.XMLChildren[1]['a'].XMLText>
                </cfif>
			...
			<cfif isDefined('thisNode.div.div') AND isDefined('thisNode.div.div.UL') AND isDefined('thisNode.div.div.UL.LI')>
				...
				<cfloop from="1" to="#arrayLen(navXMLSub1)#" index="s">
					...
					<cfelse>
                    	<cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug C<cfdump var="#thisNode#" label="thisNode"></cfmail>
                        <cfset linkXML.XMLAttributes = thisNode['A'].XMLAttributes>
					</cfif>
					...
					<cfelse>
                    	<cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug D<cfdump var="#thisNode#" label="thisNode"></cfmail>
                        <cfset thisA = thisNode['A']>
					</cfif>					
					<cfif isDefined('thisA.img')>
						<cfset imgXML = XMLElemNew(newNav,"img")>
					...
					<cfelse>
                        ...
                        <cfelse>
                            <cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug E<cfdump var="#thisNode#" label="thisNode"></cfmail>
                            <cfset linkXML.XMLText = thisNode['A'].XMLText>
                        </cfif>
						<cfset structDelete(VARIABLES,"imgXML")>
					</cfif>
					...
					<cfif isDefined('thisNode.div') AND isDefined('thisNode.div.UL') AND isDefined('thisNode.div.UL.LI')>
						...
						<cfloop from="1" to="#arrayLen(navXMLSub2)#" index="t">
							...
							<cfif StructKeyExists(thisNode,'a')>
                               <cfset linkXML.XMLAttributes = thisNode['a'].XMLAttributes>
                            <cfelse>
                                <cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug F<cfdump var="#thisNode#" label="thisNode"></cfmail>
                                <cfset linkXML.XMLAttributes = thisNode['a'].XMLAttributes>
                            </cfif>
							<!--- ERJ Debug G --->
							<cfif StructKeyExists(thisNode,'a')>
								<cfset thisA = thisNode['a']>
                            <cfelse>
                            	<cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug G<cfdump var="#thisNode#" label="thisNode"></cfmail>
                                <cfset thisA = thisNode['a']>
                            </cfif>
							<cfif isDefined('thisA.img')>
								<cfset imgXML = XMLElemNew(newNav,"img")>
							...
							<cfif StructKeyExists(thisNode,'a')>
								<cfset linkXML.XMLText = thisNode['a'].XMLText>
                            <cfelse>
                            	<cfmail to="support@d-p.com" from="support@d-p.com" type="html" subject="NavWrapper Node Error"> #Application.sitemapping# :: Error navWrapper CFC ERJ Debug H<cfdump var="#thisNode#" label="thisNode"></cfmail>
                                <cfset linkXML.XMLText = thisNode['a'].XMLText>
                            </cfif>
								<cfset structDelete(VARIABLES,"imgXML")>
							</cfif>
