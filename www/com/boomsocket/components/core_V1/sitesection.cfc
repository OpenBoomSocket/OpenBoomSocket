...
<cfcomponent displayname='sitesection' hint='sitesection CFC Bean' >
	<!--- init method --->
	<cffunction name='init' displayname='sitesectionBean init()' hint='initialze the bean' access='public' returntype='struct' output='false'>
		<cfargument name='sitesectionid' displayname='Section ID' hint='ID of the section' type='numeric' required='no' default=0 />
		...
		<cfargument name='sitesectionparent' displayname='Parent section' hint='ID of the section above this one' type='string' required='no' default=0 />
		<cfargument name='sitesectionlabel' displayname='Label' hint='Friendly label of the section' type='string' required='no' default='' />
		<cfargument name='templateid' displayname='Template ID' hint='ID of the default template used in the section' type='numeric' required='no' default=0 />
		<cfscript>
...
<!--- QUERIES --->
<!--- Retrieve section info from DB by sitesectionid --->
	<cffunction name="querySectionById" displayname="Query Section By ID" hint="Query section information by sitesectionid" access="public" returntype="query" output="false">
		<cfargument name='sitesectionid' displayname='numeric sitesectionid' hint='Id of the section to retrieve' type='numeric' required='yes' />
		<cfset var q_getSection = "">
		<cfquery name="q_getSection" datasource="#application.datasource#" username="#APPLICATION.dbUserName#" password="#APPLICATION.dbpassword#">
			SELECT * FROM sitesection WHERE sitesectionid = #arguments.sitesectionid#
		</cfquery>
		<cfreturn q_getSection>
	</cffunction>	
...
<!--- Set section info from DB to section instance using its id --->
	<cffunction name="popSectionById" displayname="Populate Section By ID" hint="Populate the section instance from the DB using sitesectionid" access="public" returntype="void" output="false">
		<cfargument name='sitesectionid' displayname='numeric sitesectionid' hint='Id of the section to retrieve' type='numeric' required='yes' default="0" />
		<cfset sectionQuery = "">
		<cfset sectionQuery = querySectionById(sitesectionid=arguments.sitesectionid)>
		<cfif sectionQuery.recordcount NEQ 0>
			<cfset setSiteSectionID(sectionQuery.sitesectionid)>
			<cfset setSiteSectionName(sectionQuery.sitesectionname)>
			<cfset setSiteSectionDesc(sectionQuery.sitesectiondesc)>
			<cfset setSiteSectionParent(sectionQuery.sitesectionparent)>
			<cfset setSiteSectionLabel(sectionQuery.sitesectionlabel)>
			<cfif len(sectionQuery.templateid)><cfset setTemplateId(sectionQuery.templateid)></cfif>
		<cfelse>
...
<!--- Check to see if section exists in system by sitesectionid --->
	<cffunction name="existsSectionById" displayname="Exists Section By ID" hint="Check to see if Section exists in system by SiteSectionid" access="public" returntype="boolean" output="false">
		<cfargument name='sitesectionid' displayname='numeric sitesection' hint='Id of the Section to retrieve' type='numeric' required='yes' />
		<cfset var pageQuery = querySectionById(sitesectionid=arguments.sitesectionid)>
		<cfreturn pageQuery.recordCount>
	</cffunction>
	
<!--- OTHER FUNCTIONS --->
<!--- Build the directory structure --->
	<cffunction name="buildDirPath" returntype="any" access="public">
		<cfargument name="currentID" required="yes" type="string">
		<cfargument name="currentDir" required="no" default="" type="string">
		<cfset var newPath = "">
		<cfset var tempStr = "">
		<cfset var q_readDir = "">
		<cfquery name="q_readDir" datasource="#application.datasource#" username="#APPLICATION.dbUserName#" password="#APPLICATION.dbpassword#">
			SELECT sitesectionparent, sitesectionname FROM sitesection WHERE sitesectionid = #currentID#
		</cfquery>
		<cfset tempStr=listPrepend(arguments.currentDir,q_readDir.sitesectionname,"/")>
		<cfif (q_readDir.recordcount NEQ 0) AND (q_readDir.sitesectionparent NEQ arguments.currentID)>
			<cfset newPath=buildDirPath(q_readDir.sitesectionparent,tempStr)>
		<cfelse>
			<cfset newPath=tempStr>
		</cfif>
		<cfreturn newPath>
	</cffunction>
<!--- Get physical file path by sitesectionid --->
	<cffunction name="physicalPathById" displayname="Physical Section path By ID" hint="Filesystem path of section" access="public" returntype="string" output="false">
		<cfargument name='sitesectionid' displayname='numeric sitesection' hint='Id of the Section to retrieve' type='numeric' required='yes' />
		<cfset var sectionPath = "#application.installpath#\#replace(buildDirPath(arguments.sitesectionid),'/','\')#\">
		<cfreturn sectionPath>
	</cffunction>
</cfcomponent>