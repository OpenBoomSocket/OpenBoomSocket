...
//	VERSION		The Flash Player version the swf is targeted for
		...
		<div id="containerDiv">
			...
			<script language="JavaScript" type="text/javascript">
				...
				if ( hasProductInstall && !hasReqestedVersion ) {
					...
					createFPObject("homeLeftPanel", "homeshow", 374, 364, "/media/swf/", "opaque", 7, [{name:"serverURL",value:"#APPLICATION.installurl#"}]);
...
function createFPObject(DivID, ObjectID, WIDTH, HEIGHT, URL, WMODE, BGCOLOR, VERSION, VARS){
	if(!(BGCOLOR.length > 0)){
		BGCOLOR = "#ffffff";		
	}
	var tempString;
	tempString = '<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version='+VERSION+',0,0,0" width="'+WIDTH+'" height="'+HEIGHT+'" id="'+ObjectID+'"><param name="allowScriptAccess" value="sameDomain" />';
	if(!(WMODE.length > 0)){
		tempString+='<param name="wmode" value="opaque" />';
	}else{
		tempString+='<param name="wmode" value="'+WMODE+'" />';
	}
	tempString+='<param name="play" value="true" /><param name="loop" value="true" />'
	if(VARS.length>0){
